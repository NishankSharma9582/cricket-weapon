...
exports.registerUser = asyncWrapper(async (req, res) => {
  ...
  if (req.body.avatar && req.body.avatar !== "") {
    ...
    // Use a default placeholder avatar when none is provided
    myCloud = {
      public_id: "default_avatar",
      secure_url: "https://res.cloudinary.com/demo/image/upload/v1615703859/default_avatar.jpg",
    };
  }
...
exports.forgotPassword = asyncWrapper(async (req, res, next) => {
  ...
  try {
    // If you have a sendEmail utility, call it here:
    // await sendEmail({
    //   email: user.email,
    //   subject: `CricketWeapon Store Password Recovery`,
    //   message,
    // });
...
exports.getUserDetails = asyncWrapper(async (req, res) => {
  ...
  const user = await userModel.findById(req.user.id); // user.id because we set that user into as user.req when user gose autentification. becauae all data of users set into req.user. only user when logged in then access this function
  res.status(200).json({
    success: true,
...
exports.updateProfile = asyncWrapper(async (req, res, next) => {
  ...
  // if avatar present then upload new image
  if (req.body.avatar && req.body.avatar !== "") {
