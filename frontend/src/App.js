...
function App() {
  ...
  useEffect(() => {
    ...
    if (storedKey) {
      // strip JSON quote wrappers left behind by older versions
      const cleaned = storedKey.replace(/^"|"$/g, "").trim();
      if (cleaned) setStripeApiKey(cleaned);
      else getStripeApiKey();
    } else {
      getStripeApiKey();
    }
  ...
  const stripePromise = useMemo(
    () => loadStripe(stripeApiKey || "pk_test_51UHpG4HvtQ5ronbq9PWhlNDSXCgP9qAcp9WxbxCzKKVzoKRGPAb3MWQH9qqEpKEH93qix1gjqEwfbQsxpl6Fh01f00QiJWFmvg"),
    [stripeApiKey]
  );
