...
// extract the real server error message (e.g. "Invalid email or password")
...
export function login(email, password) {
  ...
  return async function (dispatch) {
    try {
      ...
      dispatch({ type: LOGIN_FAIL, payload: serverError(error) });
    }
  };
}
// resgister user
export function signUp(signupData) {
  ...
  return async function (dispatch) {
    try {
      ...
      dispatch({ type: REGISTER_USER_FAIL, payload: serverError(error) })
    }
...
export const load_UserProfile = () => async (dispatch) => {
  try {
    ...
    dispatch({ type: LOAD_USER_FAIL, payload: serverError(error) });
  }
};
...
export function logout() {
  return async function (dispatch) {
    try {
      ...
      sessionStorage.removeItem("user");
      dispatch({ type: LOGOUT_FAIL, payload: serverError(error) });
    }
  }
}
  ...
  return async function (dispatch) {
    try {
     ...
     if(data.user !== undefined && data.user) {
      ...
      dispatch({ type: UPDATE_PROFILE_FAIL, payload: serverError(error) })
    }
  }
}
...
export function updatePassword(userPassWord) {
  return async function (dispatch) {
    try {
      ...
      dispatch({ type: UPDATE_PASSWORD_FAIL, payload: serverError(error) })
    }
  }
}
// forgetPassword;
...
export function forgetPassword(email) {
  return async function (dispatch) {
    try {
      ...
      dispatch({ type: FORGOT_PASSWORD_FAIL, payload: serverError(error) });
    }
  };
}
...
export const getAllUsers  = () =>async (dispatch) =>{
  ...
  try {
    ...
    dispatch({ type: ALL_USERS_SUCCESS, payload: data.users});
      ...
      dispatch({type : ALL_USERS_FAIL , payload : serverError(error)})
  }
...
export const getUserDetails = (id) => async (dispatch) => {
  try {
     ...
     dispatch({ type: USER_DETAILS_FAIL , payload : serverError(error)});
  }
}
...
export const updateUser = (id, userData) => async (dispatch) => {
  try {
    ...
    dispatch({ type: UPDATE_USER_SUCCESS, payload: data.success });
      ...
      dispatch({type : UPDATE_USER_FAIL , payload : serverError(error)} )
  }
...
export const deleteUser  =(id) => async (dispatch) =>{
  try {
      ...
      dispatch({type : DELETE_USER_FAIL , payload : serverError(error)})
  }
